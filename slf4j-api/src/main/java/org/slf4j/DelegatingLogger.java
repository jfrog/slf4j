...
/**
 * A {@link Logger} wrapper that get the actual logger on on every log action. <p/>
 * This is done in order to support multiple web applications sharing the slf4j and other libraries in the same
 * classloader. This implementation allows each web app to use it's own logger (it's own format, output file etc.)
 * even when the logger is a static member.
 * For additional info see org.jfrog.common.logging.logback.servlet.LogbackContextSelector
 *
 * @author Yoav Landman
 * @author Yossi Shaul
 */
public class DelegatingLogger implements Logger {
    ...
    public DelegatingLogger(Class clazz) {
